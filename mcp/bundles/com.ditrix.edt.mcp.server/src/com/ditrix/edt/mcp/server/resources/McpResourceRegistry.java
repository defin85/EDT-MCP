    ...
    private McpResourceRegistry()
    {
        ...
        register(new McpResource(
                ...
                markdown(
                        ...
                        "8. `get_debug_variables` reads bounded variables for a selected `frameId`.",
                        "9. `control_debug_session` resumes, suspends, steps, or terminates the addressed session or thread.",
                        "10. `terminate_debug_launch` terminates only a matching RuntimeClient launch through Eclipse termination APIs.",
                        "11. `remove_debug_breakpoint` removes MCP-owned breakpoints by `breakpointId`.",
                        "",
                        "## Live state",
                        "",
        ...
        register(new McpResource(
                ...
                markdown(
                        ...
                        "4. Call `wait_debug_session` or `list_debug_sessions` until a supported thread is visible.",
                        "5. Use `list_debug_launches` when the session is not visible and launch/process diagnostics are needed.",
                        "6. Call `get_debug_stack` with the suspended `threadId`.",
                        "7. Call `get_debug_variables` with a selected `frameId` when variable inspection is needed.",
                        "8. Call `evaluate_debug_expression` only for bounded checks on a current suspended `frameId`; responses do not guarantee side-effect-free evaluation.",
                        "9. Call `control_debug_session` for `step_over`, `step_into`, `step_return`, `resume`, `suspend`, or `terminate`.",
                        "10. Call `terminate_debug_launch` for stale or no-longer-needed matching RuntimeClient launches.",
                        "11. Call `remove_debug_breakpoint` or `cleanup_mcp_debug_breakpoints` for MCP-owned breakpoints during cleanup.",
                        "",
                        "## One-shot helper",
                        "",
