...
mutable tools: `update_database`, `clean_project`, full-project `revalidate_objects`, and
`run_unit_tests`.

#### Scenario: Client invokes an async-first tool without explicit task metadata

- **WHEN** a client calls `tools/call` for `update_database`, `clean_project`, full-project
  `revalidate_objects`, or `run_unit_tests`
...
`update_database`, `clean_project`, full-project `revalidate_objects`, and `run_unit_tests`.
...
- **WHEN** a client reads `tools/list` metadata or repository documentation for
  `update_database`, `clean_project`, full-project `revalidate_objects`, or `run_unit_tests`
