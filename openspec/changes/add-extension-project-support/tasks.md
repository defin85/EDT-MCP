...
- [ ] 2.1 Add shared project-kind, capability, and project-context resolution for EDT projects.
- [ ] 2.2 Add central capability validation so tools do not duplicate project-kind checks.

## 3. Client Discovery

- [ ] 3.1 Extend `list_projects` to expose `projectKind`, capability hints, and extension metadata
      where available without regressing existing configuration output.

## 4. Extension Metadata And Module Support

- [ ] 4.1 Migrate verified read-only metadata and module tools to the shared project context.
...
- [ ] 6.1 Update `README.md` with an honest extension support matrix and non-goals for lifecycle
      operations.
