...
- Add fail-closed live evidence guardrails for form command availability and document write/post
  dry-run safety; move document movement record reads to the follow-up
  `add-04-document-movement-live-evidence-probe` change until a proven read-only runtime transport
  exists.
...
- Affected code: validation tooling, BSL query extraction, form metadata inspection, live evidence
  guardrails, capability registry, docs/resources, focused tests
